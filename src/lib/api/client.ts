...
// navigation is instant instead of re-fetching. It never caches authenticated
// (admin) requests, and any non-GET (enquiry, admin write…) clears it, so a
...
export const apiRequest = async <T>(path: string, options: RequestOptions = {}) => {
  ...
  // A write may have changed listings — drop the anonymous cache so the next
  // GET re-fetches fresh (cheap: it only clears the in-memory client cache).
  else if (browser && method !== 'GET') getCache.clear();
