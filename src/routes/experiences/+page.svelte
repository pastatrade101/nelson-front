...
<section class="relative overflow-hidden bg-gradient-to-br from-deep-green via-forest to-deep-green text-white">
  ...
  <div class="container-shell relative py-16 text-center md:py-20">
    <span class="inline-flex items-center gap-2 rounded-full border border-goldfinch-gold/30 bg-goldfinch-gold/10 px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.18em] text-goldfinch-gold">
      <Compass size={14} strokeWidth={2.4} /> Experiences
    </span>
    <h1 class="mx-auto mt-5 max-w-3xl text-3xl font-extrabold leading-[1.1] tracking-tight md:text-[44px]">What do you want to experience?</h1>
  ...
  {:else}
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {#each experiences as exp (exp.slug)}
        <a class="group relative flex h-64 flex-col justify-end overflow-hidden rounded-2xl bg-deep-green shadow-soft" href={`/experiences/${exp.slug}`}>
          {#if exp.image_url}
