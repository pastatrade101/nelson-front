...
{:else if country}
  ...
  <section class="container-shell py-12 md:py-16">
    ...
    <div class="mt-12 overflow-hidden rounded-[10px] bg-gradient-to-br from-deep-green via-forest to-deep-green p-8 text-center text-white md:p-12">
      ...
      <a class="mt-6 inline-flex h-12 items-center gap-2 rounded-xl bg-goldfinch-gold px-7 font-bold text-heading transition hover:brightness-105" href={`/plan-my-trip?destination=${country.slug}`}>Plan My Trip <ArrowRight size={18} /></a>
    </div>
  </section>
{:else}
