  ...
  <section class="relative overflow-hidden bg-deep-green text-white">
    ...
    <div class="container-shell relative py-16 md:py-24">
      ...
      <div class="mt-6 flex flex-wrap gap-3">
        <a class="inline-flex h-12 items-center gap-2 rounded-xl bg-goldfinch-gold px-6 font-bold text-deep-green transition hover:brightness-105" href={`/plan-my-trip?destination=${country.slug}`}>
          <Sparkles size={18} /> Plan a trip to {country.name}
        </a>
        <a class="inline-flex h-12 items-center gap-2 rounded-xl border border-white/30 px-6 font-semibold text-white transition hover:bg-white/10" href="/tours">
      ...
      <div class="rounded-2xl border border-ink/10 bg-white p-5 shadow-soft">
        ...
        {#if country.best_months?.length}
          <div class="mt-3 flex flex-wrap gap-1.5">
            {#each country.best_months as m}<span class="rounded-full bg-sand/60 px-2.5 py-1 text-xs font-semibold text-deep-green">{m}</span>{/each}
          </div>
        {:else}
    ...
    {#if destinations.length}
      <div class="mt-12">
        <div class="flex flex-wrap items-end justify-between gap-4">
          <h2 class="text-2xl font-bold text-deep-green md:text-3xl">Where to go in {country.name}</h2>
          <a class="inline-flex items-center gap-1.5 text-sm font-semibold text-forest transition hover:text-deep-green" href="/destinations">All destinations <ArrowRight size={16} /></a>
        </div>
...
{:else}
  <section class="container-shell py-20 text-center">
    <h1 class="text-2xl font-bold text-deep-green">Country not found</h1>
    <a class="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-forest hover:text-deep-green" href="/countries">All countries <ArrowRight size={16} /></a>
  </section>
{/if}
